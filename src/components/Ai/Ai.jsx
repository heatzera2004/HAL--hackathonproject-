    ...
    try {
      if (model && (imageRef.current || webcamRef.current)) {
        ...
        if (topPrediction) {
          ...
          const crop = selectedCrop.toLowerCase(); // Use the selected crop from dropdown
            ...
            <div className="flex space-x-4 w-full mt-6">
              <button
                ...
                className={`w-full py-3 px-6 rounded-lg font-bold text-white ${
                  !imageSrc || isLoading || isPredicting ? 'bg-gray-400' : 'bg-green-600'
                } shadow-lg transform transition-transform duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500`}
              >
          ...
          <div className="mt-8">
            ...
            {remedy && (
              <div className="mt-4">
                ...
                <p className="text-lg text-gray-700">{remedy.en}</p>
                <p className="text-lg text-gray-700">{remedy.hi}</p>
              </div>
            )}
          </div>
        )}
...
// Remedies based on crop and disease
const remedies = {
  "wheat": {
    "Brown Rust": {
